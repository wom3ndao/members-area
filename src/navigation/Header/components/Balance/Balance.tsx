...
function Balance() {
  ...
  const formatNumber = useMemo(
    () => (tokenInfo?.type === "erc721" || tokenInfo?.type === "erc5484" ? 0 : 2),
    [tokenInfo]
  );
      ...
      trigger={
        <Button alwaysEnabled look="secondary">
          <div className="balance">
            <p className="text-lg color-primary font-semibold">{formatNumberCompact(totalBalance, formatNumber)}</p>
    ...
    >
      <div className="balance-content">
        <div className="balance-q">
          ...
          <p className="text-xl color-primary font-semibold">{formatNumberCompact(totalBalance, formatNumber)}</p>
        </div>
