  ...
  return (
    <StyledWrapper className="block">
      <div className="block__header dao-token-supply__header">
        <h2 className="text-lg">{"Gemintete DAO NFTs"}</h2>

        {(isErc20Token || isTokenOwner) && (
          <Dropdown
            right
            open={isMenuOpen}
            trigger={
              <Button icon className="dao-token-supply__menu-btn" look="ghost" active={isMenuOpen}>
          ...
          >
            <div className="dao-token-supply__menu">
              {isErc20Token && (
                <button className="dao-token-supply__menu-item text-md" onClick={addTokenToWallet}>
                  <img className="dao-token-supply__menu-icon" src="/icons/metamask.svg" alt="metamask" />
                  <span>{t("ADD_TO_WALLET")}</span>
                </button>
              )}
              {isTokenOwner && (
                <button className="dao-token-supply__menu-item text-md" onClick={() => setIsModalOpen(true)}>
                  <Icon name="coins" />
                  {t("MINT_TOKENS")}
                </button>
              )}
        ...
        {!tokenInfo || tokenInfo.type === "native" ? (
          ...
          <>
            <p className="text-xl font-semibold" title={fromWeiWithDecimals(tokenInfo.totalSupply, tokenInfo.decimals)}>
              {formatNumberCompact(
                fromWeiWithDecimals(tokenInfo.totalSupply, tokenInfo.decimals),
      ...
      <Modal open={isModalOpen} title={t("MINT")} onClose={() => setIsModalOpen(false)}>
